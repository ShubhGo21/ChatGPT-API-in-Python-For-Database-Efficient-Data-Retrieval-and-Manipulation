# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 10:32:00 2023
...
class Application(tk.Frame):
    ...
    def MongoDB(self):
        ...
        query_string = re.sub(r":\s*\{\s*(\$lt|\$gt)\s*:\s*(\d+)\s*\}\s*", r': {"\1": \2}', m1)
        print(query_string)
        
        d_name, collection_and_query = query_string.split('.', 1)
        collection_name, query_and_projection = collection_and_query.split('(')
        query_and_projection = query_and_projection.rstrip(')')
        query_dict = query_and_projection.split(',', 1)[0]
        
        collection_name = collection_name.replace('.find', '')

        # Convert the query and projection strings to dictionaries using json.loads()
        query_dict = json.loads(query_dict)
        try:
            projection_dict = json.loads(query_and_projection.split(',', 1)[1])
        except:
        ...
        # Execute the query using find() method on the collection object
        if projection_dict:
            result = collection.find(query_dict, projection_dict)
        else:
        ...
        # Use pandasgui to display the DataFrame in a tkinter window
        pg.show(df)
